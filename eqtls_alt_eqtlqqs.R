...
library('qvalue')
source('./config.R')
bounddir = '/mnt/lustre/home/cusanovich/Kd_Arrays/Trans_eQTLs/AlteQTLs/Bound/'
dedir = '/mnt/lustre/home/cusanovich/Kd_Arrays/Trans_eQTLs/AlteQTLs/DEandBound/'
unbounddir = '/mnt/lustre/home/cusanovich/Kd_Arrays/Trans_eQTLs/AlteQTLs/Unbound/'
realcis = '/mnt/lustre/home/cusanovich/Kd_Arrays/Trans_eQTLs/RealCis/'

qqer = function(de,bound,unbound,name){
  obsp <- -log10(de)
  N <- length(obsp)
  ...
  plot(1,1, type="n", xlim=c(0,max(null)+0.5),ylim=c(0,max(obsp,-log10(bound),-log10(unbound))+1.5),
       xlab="Expected", ylab="Observed",main=name,las=1)
  polygon(xx, yy, col="gray",border="gray")
  lines(null,null,lwd=2)
  points(sort(null),sort(-log10(unbound)),pch=20,col="mediumorchid3")
  points(sort(null),sort(obsp),pch=20,col="indianred")
  points(sort(null),sort(-log10(bound)),pch=20,col="dodgerblue2")
  legend("topleft",legend=c("Candidate Genes","Bound\nControl Genes","Unbound\nControl Genes"),
         col=c("indianred","mediumorchid3","dodgerblue2"),pch=20)
}

all.boundeqtls = list.files(path = realcis,pattern="_100kb_sidney_results.txt")
