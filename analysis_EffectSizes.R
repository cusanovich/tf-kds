...
for(i in 4:dim(master[[3]])[2]){
  ...
  matrixcol = match(matchgene,colnames(resultsmatrix))
  ...
  boundgenes = rownames(resultsmatrix)[resultsmatrix[,matrixcol] > 0]
  currbound = match(boundgenes,master[[1]][,2])
  currbound = currbound[!is.na(currbound)]
  bounddirections[[length(bounddirections)+1]] = master[[3]][currbound[master[[2]][currbound,i] < 0.05],i]
  names(bounddirections)[length(bounddirections)] = matchgene
  boundups = c(boundups,length(which(master[[3]][currbound[master[[2]][currbound,i] < 0.05],i] > 0))/length(master[[3]][currbound[master[[2]][currbound,i] < 0.05],i]))
}
