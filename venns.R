library('VennDiagram')
bindingmatrix = read.table("/mnt/lustre/home/cusanovich/Kd_Arrays/CombinedBinding/Results/5RUV2_NSAveraged_FACTOR_10kbBindingWindow_overlaptable.txt",sep="\t",header=T)
pdf("venns.pdf")
for(i in 1:dim(bindingmatrix)[1]){
  ...
  grid.draw(draw.pairwise.venn(as.numeric(bindingmatrix[i,11]),
                               ...
                               alpha=c(.8,.6),cex=2,cat.cex=2,cat.dist=c(0.1,0.1),cat.just=list(c(-1,-1),c(1,1))))
}
dev.off()